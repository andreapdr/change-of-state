...
def create_foils(sentence, foil_types=["action"]):
    ...
    for ft in foil_types:
        foils[ft] = switcher[ft](sentence, target="verb-hypernym")

    return {"foils": foils}


def get_action_capt_and_foil(sentence, target):
    ...
    if not_transitive(sentence[target]):
        if REDUCED:
            ...
            foil = f"Initially, {_object} {aux} {sentence['pre-state']}. Then, {_object} {_inverse}{' ' + _particle_inv if _particle_inv else ''}. At the end, {_object} {aux} {sentence['post-state']}."
    else:
        if REDUCED:
            ...
            capt = f"Initially, {_object} {aux} {sentence['pre-state']}. Then, someone {_verb_passive} {_object}{' ' + _particle_passive if _particle_passive else ''}. At the end, {_object} {aux} {sentence['post-state']}."
            foil = f"Initially, {_object} {aux} {sentence['pre-state']} Then, someone {_inverse_passive} {_object}{' ' + _particle_inv_passive if _particle_inv_passive else ''}. At the end, {_object} {aux} {sentence['post-state']}."
    return capt, foil


def get_prestate_capt_and_foil(sentence, target):
    ...
    if not_transitive(sentence[target]):
        if REDUCED:
            ...
            # EXTENDED VERSION
            capt = f"Initially, {_object} {aux} {sentence['pre-state']}. Then, {_object} {_verb}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['post-state']}."
            foil = f"Initially, {_object} {aux} {sentence['post-state']}. Then, {_object} {_verb}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['post-state']}."
    else:
        if REDUCED:
            # REDUCED VERSION
            capt = f"Initially, {_object} {aux} {sentence['pre-state']}."
            ...
            foil = f"Initially, {_object} {aux} {sentence['post-state']} Then, someone {_verb} {_object}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['post-state']}."
    return capt, foil


def get_poststate_capt_and_foil(sentence, target):
    _object = sentence["object"] if sentence["object"] else "Someone"
    _verb, _particle = inflect(sentence[target], tag="VBZ")
    aux = manage_aux(_object)
    ...
    if not_transitive(sentence[target]):
        if REDUCED:
            ...
            # EXTENDED VERSION
            capt = f"Initially, {_object} {aux} {sentence['pre-state']}. Then, {_object} {_verb}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['post-state']}."
            foil = f"Initially, {_object} {aux} {sentence['pre-state']}. Then, {_object} {_verb}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['pre-state']}."
    else:
        if REDUCED:
            capt = f"At the end, {_object} {aux} {sentence['post-state']}."
            foil = f"At the end, {_object} {aux} {sentence['pre-state']}."
        ...
        # EXTENDED VERSION
            ...
            capt = f"Initially, {_object} {aux} {sentence['pre-state']}. Then, someone {_verb} {_object}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['post-state']}."
            foil = f"Initially, {_object} {aux} {sentence['pre-state']} Then, someone {_verb} {_object}{' ' + _particle if _particle else ''}. At the end, {_object} {aux} {sentence['pre-state']}."
    return capt, foil


def get_inverse_capt_and_foil(sentence, target):
    _verb, _particle = inflect(sentence[target], tag="VBZ")
...
if __name__ == "__main__":
    ...
    # foil_types = ["pre-state"]

    datapath = expanduser("~/datasets/vl-bench/change-state.json")
    outpath = expanduser(
        "~/datasets/vl-bench/change-state-extended-hypernyms-only.json"
    )
